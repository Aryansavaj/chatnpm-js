...
import { isNotDefined } from "@chat77npm/lib/utils";
...
export const ConversationContainer = (props: Props) => {
  ...
  const [chatChunks, setChatChunks, isRecovered, setIsRecovered] = persist(
    ...
    {
      key: `chat77submodule-${props.context.chat77submodule.id}-chatChunks`,
  ...
  const [theme, setTheme] = createSignal(props.initialChatReply.chat77submodule.theme);
  ...
  createEffect(() => {
    setTheme(
      parseDynamicTheme(props.initialChatReply.chat77submodule.theme, dynamicTheme()),
    );
  });
  ...
  return (
    <div
      ref={chatContainer}
      class="flex flex-col overflow-y-auto w-full px-3 pt-10 relative scrollable-container chat77submodule-chat-view scroll-smooth gap-2"
    >
      <For each={chatChunks()}>
        {(chatChunk, index) => (
          <ChatChunk
            ...
            theme={theme()}
            settings={props.initialChatReply.chat77submodule.settings}
...
const BottomSpacer = () => (
  <div
    class="w-full flex-shrink-0 chat77submodule-bottom-spacer"
