...
export const StripePaymentForm = (props: Props) => {
      ...
      appearance: {
        ...
        variables: {
          colorPrimary: getComputedStyle(paymentElementSlot).getPropertyValue(
            "--chat77submodule-button-bg-color",
          ),
        },
      },
  ...
  const handleSubmit = async (event: Event & { submitter: HTMLElement }) => {
    ...
    setPaymentInProgressInStorage({
      ...
      chat77submodule: props.context.chat77submodule,
    });
  ...
  return (
    <form
      ...
      class="flex flex-col p-4 chat77submodule-input w-full items-center"
    >
      ...
      <Show when={message()}>
        <div class="chat77submodule-input-error-message mt-4 text-center animate-fade-in">
          {message()}
        </div>
      </Show>
    </form>
